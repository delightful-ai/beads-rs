...
use rustc_hir::{
    self as hir, AmbigArg, AnonConst, Arm, Block, BlockCheckMode, Body, BodyId, Expr, ExprKind,
    HirId, ItemId, ItemKind, LetExpr, Pat, QPath, Stmt, StructTailExpr, UnOp, UnsafeSource,
};
...
pub fn for_each_expr<'tcx, B, C: Continue>(
    ...
    impl<'tcx, B, C: Continue, F: FnMut(&'tcx Expr<'tcx>) -> ControlFlow<B, C>> Visitor<'tcx>
        for V<'tcx, F>
    {
...
fn contains_try(expr: &Expr<'_>) -> bool {
    for_each_expr_without_closures(expr, |e| {
        if matches!(
            e.kind,
            ExprKind::Match(_, _, hir::MatchSource::TryDesugar(_))
        ) {
            ControlFlow::Break(())
        } else {
            ControlFlow::Continue(())
        ...
        fn visit_expr(&mut self, e: &'tcx Expr<'_>) -> Self::Result {
            match e.kind {
                ...
                ExprKind::Unary(UnOp::Deref, e)
                    if self.cx.typeck_results().expr_ty(e).is_raw_ptr() =>
                {
                    ()
                }
                ExprKind::Unary(_, e)
            ...
            match e.kind {
                ...
                ExprKind::MethodCall(..)
                    if self
                        ...
                        .type_dependent_def_id(e.hir_id)
                        .is_some_and(|id| {
                            self.cx.tcx.fn_sig(id).skip_binder().safety().is_unsafe()
                        }) =>
                {
                    ControlFlow::Break(())
                }
                ExprKind::Call(func, _) => {
                    match *self.cx.typeck_results().expr_ty(func).peel_refs().kind() {
                        ty::FnDef(id, _)
                            if self.cx.tcx.fn_sig(id).skip_binder().safety().is_unsafe() =>
                        {
                            ControlFlow::Break(())
                        }
