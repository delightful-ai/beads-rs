...
pub use self::hir_utils::{
    HirEqInterExpr, SpanlessEq, SpanlessHash, both, count_eq, eq_expr_value,
...
use rustc_hir::{
    self as hir, Arm, BindingMode, Block, BlockCheckMode, Body, ByRef, Closure, ConstArgKind,
    CoroutineDesugaring, CoroutineKind, CoroutineSource, Destination, Expr, ExprField, ExprKind,
    FnDecl, FnRetTy, GenericArg, GenericArgs, HirId, Impl, ImplItem, ImplItemKind, Item, ItemKind,
    LangItem, LetStmt, MatchSource, Mutability, Node, OwnerId, OwnerNode, Param, Pat, PatExpr,
    PatExprKind, PatKind, Path, PathSegment, QPath, Stmt, StmtKind, TraitFn, TraitItem,
...
use rustc_middle::ty::{
    self as rustc_ty, Binder, BorrowKind, ClosureKind, EarlyBinder, GenericArgKind, GenericArgsRef,
    IntTy, Ty, TyCtxt, TypeFlags, TypeVisitableExt, UintTy, UpvarCapture,
};
...
pub fn is_in_const_context(cx: &LateContext<'_>) -> bool {
    debug_assert!(
        cx.enclosing_body.is_some(),
        "`LateContext` has no enclosing body"
    );
...
pub fn is_ty_alias(qpath: &QPath<'_>) -> bool {
    match *qpath {
        QPath::Resolved(_, path) => {
            matches!(path.res, Res::Def(DefKind::TyAlias | DefKind::AssocTy, ..))
        }
...
pub fn path_to_local_with_projections(expr: &Expr<'_>) -> Option<HirId> {
    match expr.kind {
        ExprKind::Field(recv, _) | ExprKind::Index(recv, _, _) => {
            path_to_local_with_projections(recv)
        }
...
/// }
/// ```
pub fn trait_ref_of_method<'tcx>(
    cx: &LateContext<'tcx>,
    owner: OwnerId,
...
pub fn can_mut_borrow_both(cx: &LateContext<'_>, e1: &Expr<'_>, e2: &Expr<'_>) -> bool {
    ...
    if expr_custom_deref_adjustment(cx, r1).is_some()
        || expr_custom_deref_adjustment(cx, r2).is_some()
    {
        return false;
    }

    for (x1, x2) in zip(&s1, &s2) {
        if expr_custom_deref_adjustment(cx, x1).is_some()
            || expr_custom_deref_adjustment(cx, x2).is_some()
        {
            return false;
        }
        ...
        && let Some(adt) = cx.tcx.type_of(impl_did).instantiate_identity().ty_adt_def()
    {
        return Some(adt.did()) == cx.tcx.lang_items().string()
            || (cx.tcx.get_diagnostic_name(adt.did()))
                .is_some_and(|adt_name| std_types_symbols.contains(&adt_name));
    }
    false
}
...
pub fn is_default_equivalent_call(
    ...
    let Some(e) = whole_call_expr else {
        return false;
    };
    ...
    let instance =
        rustc_ty::Instance::try_resolve(cx.tcx, cx.typing_env(), default_fn_def_id, args);

    let Ok(Some(instance)) = instance else {
        return false;
    };
...
pub fn is_default_equivalent(cx: &LateContext<'_>, e: &Expr<'_>) -> bool {
    match &e.kind {
        ...
        ExprKind::Tup(items) | ExprKind::Array(items) => {
            items.iter().all(|x| is_default_equivalent(cx, x))
        }
        ...
        ExprKind::Block(
            ...
            _,
        ) => expr.is_some_and(|e| is_default_equivalent(cx, e)),
        _ => false,
    }
}
...
impl std::ops::BitOr for CaptureKind {
    ...
    fn bitor(self, rhs: Self) -> Self::Output {
        match (self, rhs) {
            ...
            | (CaptureKind::Ref(_), CaptureKind::Ref(Mutability::Mut)) => {
                CaptureKind::Ref(Mutability::Mut)
            }
            (CaptureKind::Ref(Mutability::Not), CaptureKind::Ref(Mutability::Not)) => {
    ...
    for (parent_id, parent) in cx.tcx.hir_parent_iter(e.hir_id) {
        ...
        match parent {
            Node::Expr(e) => match e.kind {
                ...
                ExprKind::Index(..) | ExprKind::Unary(UnOp::Deref, _) => {
                    capture = CaptureKind::Ref(Mutability::Not)
                }
                ExprKind::Assign(lhs, ..) | ExprKind::AssignOp(_, lhs, _)
                    if lhs.hir_id == child_id =>
                {
...
pub fn can_move_expr_to_closure<'tcx>(
    cx: &LateContext<'tcx>,
    expr: &'tcx Expr<'_>,
    ...
    impl<'tcx> Visitor<'tcx> for V<'_, 'tcx> {
        fn visit_expr(&mut self, e: &'tcx Expr<'_>) {
            ...
            match e.kind {
                ...
                _ => {
                    self.allow_closure &=
                        can_move_expr_to_closure_no_visit(self.cx, e, &self.loops, &self.locals);
                    walk_expr(self, e);
                }
            }
        }
...
pub fn parent_item_name(cx: &LateContext<'_>, expr: &Expr<'_>) -> Option<Symbol> {
    ...
    match cx.tcx.hir_node_by_def_id(parent_id) {
        ...
        Node::TraitItem(TraitItem { ident, .. }) | Node::ImplItem(ImplItem { ident, .. }) => {
            Some(ident.name)
        }
        _ => None,
    }
...
pub fn is_refutable(cx: &LateContext<'_>, pat: &Pat<'_>) -> bool {
    ...
    match pat.kind {
        ...
        PatKind::Struct(ref qpath, fields, _) => {
            is_qpath_refutable(cx, qpath, pat.hir_id)
...
pub fn is_try<'tcx>(cx: &LateContext<'_>, expr: &'tcx Expr<'tcx>) -> Option<&'tcx Expr<'tcx>> {
    ...
    if let ExprKind::Match(_, arms, ref source) = expr.kind {
        ...
        if arms.len() == 2
            ...
            && ((is_ok(cx, &arms[0]) && is_err(cx, &arms[1]))
    ...
    match (pat.kind, expr.kind) {
            ...
            if dotdot.as_opt_usize().is_none() && pats.len() == tup.len() =>
        {
            over(pats, tup, |pat, expr| {
                is_expr_identity_of_pat(cx, pat, expr, by_hir)
            })
        }
        (PatKind::Slice(before, None, after), ExprKind::Array(arr))
            if before.len() + after.len() == arr.len() =>
        {
        ...
        (
            PatKind::Struct(pat_ident, field_pats, None),
...
pub fn is_no_std_crate(cx: &LateContext<'_>) -> bool {
    find_attr!(
        cx.tcx.hir_attrs(hir::CRATE_HIR_ID),
        AttributeKind::NoStd(..)
    )
}
...
static TEST_ITEM_NAMES_CACHE: OnceLock<Mutex<FxHashMap<LocalModDefId, Vec<Symbol>>>> =
    OnceLock::new();

/// Apply `f()` to the set of test item names.
/// The names are sorted using the default `Symbol` ordering.
fn with_test_item_names(
    tcx: TyCtxt<'_>,
    ...
    pub fn use_node(&self, cx: &LateContext<'tcx>) -> ExprUseNode<'tcx> {
        match self.node {
            ...
            Node::Expr(use_expr) => match use_expr.kind {
                ...
                ExprKind::Closure(closure) => ExprUseNode::Return(OwnerId {
                    def_id: closure.def_id,
                }),
                ExprKind::Call(func, args) => {
                    match args.iter().position(|arg| arg.hir_id == self.child_id) {
...
pub fn expr_use_ctxt<'tcx>(cx: &LateContext<'tcx>, e: &Expr<'tcx>) -> ExprUseCtxt<'tcx> {
    ...
    let node = walk_to_expr_usage(cx, e, &mut |parent_id,
                                               parent,
                                               child_id|
     -> ControlFlow<!> {
...
pub fn is_never_expr<'tcx>(cx: &LateContext<'tcx>, e: &'tcx Expr<'_>) -> Option<RequiresSemi> {
        ...
        && b.expr.is_none()
    {
        ...
        v.visit_expr(e);
        v.is_never.then_some(
            if v.requires_semi && matches!(e.kind, ExprKind::Block(..)) {
                RequiresSemi::Yes
            } else {
                RequiresSemi::No
            },
    ...
    match expr.kind {
            ...
            if let Some(def_id) = fn_def_id(cx, expr) =>
        {
            ...
            if !fn_sig
                .output()
                .skip_binder()
                .has_type_flags(TypeFlags::HAS_TY_PARAM)
            {
                return false;
            }
        ...
        ExprKind::Array(elems) | ExprKind::Tup(elems) => {
            elems.iter().any(|elem| expr_requires_coercion(cx, elem))
        }
        ...
        ExprKind::If(_, then, maybe_else) => {
            expr_requires_coercion(cx, then)
                || maybe_else.is_some_and(|e| expr_requires_coercion(cx, e))
        }
        ...
        match node {
            ...
            Node::Expr(expr) => match expr.kind {
                ...
                ExprKind::If(_, then, opt_else)
                    if then.hir_id == prev_id
                        || opt_else.is_some_and(|els| els.hir_id == prev_id) => {}
