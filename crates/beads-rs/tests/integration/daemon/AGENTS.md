...
- Copy `repl_e2e.rs` only for multi-daemon external-process replication, tailnet proxy faults, or checkpoint/WAL behavior that truly needs the assembly harness.
...
- Treat `repl_e2e.rs` as the constrained slow/tailnet zone, not the default template for ordinary daemon tests.
