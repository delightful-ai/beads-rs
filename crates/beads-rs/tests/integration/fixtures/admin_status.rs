...
pub fn assert_monotonic_watermarks(samples: &[AdminStatusOutput]) {
    let mut prev_applied: Option<Watermarks<beads_rs::core::Applied>> = None;
    let mut prev_durable: Option<Watermarks<beads_rs::core::Durable>> = None;
...
fn is_require_min_seen_timeout(err: &beads_rs::core::ErrorPayload) -> bool {
    matches!(
        err.code,
...
fn next_applied_requirement(
    ...
    read: &ReadConsistency,
) -> Result<Watermarks<beads_rs::core::Applied>, StatusError> {
