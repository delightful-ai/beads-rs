...
mod tests {
    ...
    fn bead_ref(namespace: &str, id: &str) -> BeadRef {
        BeadRef::new(
            NamespaceId::parse(namespace).unwrap(),
    ...
    fn make_key() -> DepKey {
        DepKey::new(
            bead_ref("core", "bd-abc"),
            bead_ref("core", "bd-xyz"),
            DepKind::Blocks,
        )
        .unwrap()
    }
    ...
    #[test]
    fn same_id_in_different_namespaces_is_not_self_dependency() {
        let key = DepKey::new(
            bead_ref("core", "bd-abc"),
            bead_ref("sessions", "bd-abc"),
            DepKind::Blocks,
        )
        .unwrap();

        assert_eq!(key.from_ref().to_string(), "core/bd-abc");
        assert_eq!(key.to_ref().to_string(), "sessions/bd-abc");
        assert_eq!(key.from_id(), key.to_id());
        assert_ne!(key.from_ref(), key.to_ref());
    }
    ...
    fn accessors_work() {
        ...
        assert_eq!(edge.child_ref().to_string(), "core/bd-abc");
        assert_eq!(edge.parent_ref().to_string(), "core/bd-xyz");
    }
    ...
    fn serde_rejects_self_dep() {
        ...
        let json = r#"{"from":{"namespace":"core","id":"bd-abc"},"to":{"namespace":"core","id":"bd-abc"},"kind":"blocks"}"#;
